...
(* these versions will not type-check without type annotations because the type-checker cannot
figure out if there might be other fields *)
...
fun partial_name {first=x, middle=y, last=z} =
  ...
  val j = partial_sum (3,4,5); (* 8 *)
  val k = partial_sum (3,"hi",5); (* 8 *)